...
public class frmTabDocCanc : Form
{
	...
	public frmTabDocCanc(string pTitle, Color pColumnColor, string pFeatExtId)
	{
		InitializeComponent();
		Text = pTitle;
	...
	private async void frmTabDocCanc_Load(object sender, EventArgs e)
	{
		...
		{
			clsScreenGeral.funcCheckListViewDdic(typeof(DocEventCancel), lsvData);
		}
	...
	private void funcDefineListViewFeatures()
	{
		...
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return (docEventCancel == null) ? null : ((object)clsScreenGeral.funcGetDocXmlIcon(docEventCancel, _HasNFSeFeatEnabled, _HasCFeFeatEnabled));
		};
		...
		olvStatus.AspectGetter = delegate(object x)
		{
			new List<int>();
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return (docEventCancel == null) ? null : clsScreenGeral.funcGetDocStatIcon(docEventCancel);
		};
		olvTipo.AspectGetter = delegate(object x)
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return (docEventCancel == null) ? null : clsFunction.funcGetDocType(docEventCancel.Model);
		};
		olvTag.ImageGetter = delegate(object x)
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return (docEventCancel == null) ? null : ((object)clsScreenGeral.funcGetDocNotesIcon(docEventCancel));
		};
		olvTag.AspectGetter = delegate(object x)
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			if (docEventCancel == null)
			{
				return (object)null;
			}
			return clsFunction.IsEmpty(docEventCancel.Tag) ? null : _TagHsList[docEventCancel.Tag];
		};
		olvNum.AspectGetter = delegate(object x)
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return (docEventCancel == null) ? null : clsFunction.funcGetValue(docEventCancel.Num).PadLeft(9, '0');
		};
		olvDtAut.AspectGetter = delegate(object x)
		{
			...
			try
			{
				result = docEventCancel.DtAut.Substring(0, 7);
			}
			catch
			{
			}
		...
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return clsFunction.IsEmpty(docEventCancel?.Valor) ? null : ((object)clsFunction.funcConvStrToDec(docEventCancel.Valor));
		};
		olvReason.AspectGetter = delegate(object x)
		{
			...
			if (!clsFunction.IsEmpty(docEventCancel.EvtxJust))
			{
				return docEventCancel.EvtxJust;
			}
			if (!clsFunction.IsEmpty(docEventCancel.EvtxMotivo))
			{
				return docEventCancel.EvtxMotivo;
			}
			return (!clsFunction.IsEmpty(docEventCancel.EvtxObs)) ? docEventCancel.EvtxObs : string.Empty;
		};
		olvCNPJ.AspectGetter = delegate(object x)
		{
			...
			if (string.IsNullOrEmpty(docEventCancel.Emitida))
			{
				return docEventCancel.EmitID;
			}
			if (!string.IsNullOrEmpty(docEventCancel.TomaID))
			{
				return docEventCancel.TomaID;
			}
			return (!string.IsNullOrEmpty(docEventCancel.DestinID)) ? docEventCancel.DestinID : docEventCancel.RecebeID;
		};
		olvNome.AspectGetter = delegate(object x)
		{
			...
			if (string.IsNullOrEmpty(docEventCancel.Emitida))
			{
				return docEventCancel.EmitNome;
			}
			if (!string.IsNullOrEmpty(docEventCancel.TomaNome))
			{
				return docEventCancel.TomaNome;
			}
			return (!string.IsNullOrEmpty(docEventCancel.DestinNome)) ? docEventCancel.DestinNome : docEventCancel.RecebeNome;
		};
		olvFilial.AspectGetter = delegate(object x)
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return (docEventCancel == null) ? null : clsFunction.funcFormatDoc(docEventCancel.Filial);
		};
		olvDias.AspectGetter = delegate(object x)
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return (docEventCancel == null) ? null : funcGetDays(docEventCancel.DtAut, docEventCancel.EvtDtAut);
		};
		olvError.AspectGetter = delegate(object x)
		{
			DocEventCancel docEventCancel = (DocEventCancel)x;
			return (docEventCancel == null) ? null : varclsValidator.funcGetDesc(docEventCancel);
		};
	...
	{
		DocEventCancel varDocument = (DocEventCancel)e.Model;
		if (varDocument != null)
		{
			_ = string.Empty;
	...
	public async Task<clsReturn> funcLoadDataAsync(clsDataFilter pclsDataFilter)
	{
		...
		List<DocEventCancel> varclsDocList = await new clsDataDocEventCancel().funcGetListByFullSqlAsync(varSqlQuery, varPageSize);
	...
	{
		List<DocEventCancel> varObjList = funcGetObjList(pFocused: true, pChecked: true);
	...
	public async Task<bool> funcSetTagAsync(string pTagCode, bool pFocused, bool pChecked)
	{
		...
		List<DocEventCancel> varObjList = funcGetObjList(pFocused, pChecked);
		List<Document> varDocList = await funcGetDocListAsync(varObjList, pSyncFromDbaFirst: true);
		if (varDocList == null)
		{
			return false;
	...
	private string funcGetDays(string pDate01, string pDate02)
	{
		DateTime varDate01 = clsFunction.funcGetDate(pDate01);
		return clsFunction.funcGetDate(pDate02).Subtract(varDate01).TotalDays.ToString();
	}
	...
	private async void funcRefreshListAsync(List<Document> pDocList, List<DocEventCancel> pObjList)
	{
		ConcurrentBag<ObjFieldBuffer> varDocFieldList = clsObjectBuffer.funcGet<Document>().Fields;
		ConcurrentBag<ObjFieldBuffer> varObjFieldList = clsObjectBuffer.funcGet<DocEventCancel>().Fields;
		await Task.WhenAll(((IEnumerable<DocEventCancel>)pObjList).Select((Func<DocEventCancel, Task>)async delegate(DocEventCancel varclsObjItem)
		{
	...
	{
		return await clsScreenGeral.funcExportDocToExcelAsync<DocEventCancel>(lsvData, onProgressChange);
	}
	...
	private async Task<List<Document>> funcGetDocListAsync(List<DocEventCancel> pObjList, bool pSyncFromDbaFirst)
	{
		List<Document> varDocList = new List<Document>();
		foreach (DocEventCancel varObject in pObjList)
		{
	...
	private List<DocEventCancel> funcGetObjList(bool pFocused, bool pChecked)
	{
		List<DocEventCancel> varDocList = new List<DocEventCancel>();
		DocEventCancel varFocused = new DocEventCancel();
		if (pFocused && lsvData.FocusedItem != null)
		{
			try
			{
				varFocused = (DocEventCancel)lsvData.GetItem(lsvData.FocusedItem.Index).RowObject;
				varDocList.Add(varFocused);
			}
			catch
			{
				varFocused = new DocEventCancel();
			}
		}
		if (pChecked)
		{
			foreach (DocEventCancel varObject in lsvData.CheckedObjects)
			{
			...
			if (varDocList.Count > 0)
			{
				varDocList = lsvData.Objects.Cast<DocEventCancel>().Intersect(varDocList).ToList();
			}
		}
		if (!pFocused && !pChecked)
		{
			varDocList = lsvData.FilteredObjects.Cast<DocEventCancel>().ToList();
			if (varDocList.Count <= 0)
			{
				varDocList = lsvData.Objects.Cast<DocEventCancel>().ToList();
			}
		}
		return varDocList;
	}
	...
	private void lsvData_FormatCell(object sender, FormatCellEventArgs e)
	{
		...
		else if (e.ColumnIndex == olvCanc.Index)
		{
			e.SubItem.BackColor = Color.FromArgb(255, 220, 188);
		}
		else if (e.ColumnIndex == olvDias.Index)
		{
			e.SubItem.BackColor = Color.FromArgb(255, 220, 188);
		}
	...
	private async void lsvData_MouseDown(object sender, MouseEventArgs e)
	{
		...
		bool varHasDummy = (await funcGetDocListAsync(pFocused: true, pChecked: true, pSyncFromDbaFirst: true)).Any((Document r) => clsFunction.IsEqual(r.Filial, varFilialDummnyIdnt));
	...
	private void funcSetTabDataTotalValues()
	{
		if (!_IsOnHeaderCheckStatus)
		{
			...
			List<DocEventCancel> varObjList = varEnumList.Cast<DocEventCancel>().ToList();
			if (varObjList == null)
			{
				varObjList = new List<DocEventCancel>();
			}
			_TotalValue = varObjList.Sum((DocEventCancel r) => clsFunction.funcConvStrToDec(r.Valor));
			EventTabManagerEventArgs varArguments = new EventTabManagerEventArgs();
	...
	{
		clsHelpService.funcCallTipReportDocCanceledAsync();
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Monitor.frmTabDocCanc));
		...
		this.olvCanc = new BrightIdeasSoftware.OLVColumn();
		this.olvDias = new BrightIdeasSoftware.OLVColumn();
		...
		this.lsvData.AllColumns.Add(this.olvCanc);
		this.lsvData.AllColumns.Add(this.olvDias);
		...
		{
			this.olvSelect, this.olvHasXml, this.olvTipo, this.olvTag, this.olvNum, this.olvSerie, this.olvDtAut, this.olvValor, this.olvStatus, this.olvDtRegFisc,
			this.olvCanc, this.olvDias, this.olvReason, this.olvCNPJ, this.olvNome, this.olvChave, this.olvFilial, this.olvCFOP, this.olvNCM, this.olvError,
			this.olvDocNote, this.olvEstado, this.olvAnoMes
		});
		...
		this.olvDtRegFisc.Text = "RegFiscal";
		this.olvDtRegFisc.TextAlign = System.Windows.Forms.HorizontalAlignment.Center;
		this.olvDtRegFisc.Width = 71;
		this.olvCanc.AspectName = "EvtDtAut";
		this.olvCanc.Text = "Dt.Canc.";
		this.olvCanc.TextAlign = System.Windows.Forms.HorizontalAlignment.Center;
		this.olvCanc.Width = 80;
		this.olvDias.Text = "Dias";
		this.olvDias.TextAlign = System.Windows.Forms.HorizontalAlignment.Center;
		this.olvDias.Width = 40;
		this.olvReason.AspectName = "EvtxJust";
		this.olvReason.Text = "Motivo do Cancelamento";
		...
		this.ImageListDocs.Images.SetKeyName(19, "image_status_exceeded.png");
		...
		this.btSalesContact.TabIndex = 215;
		...
		this.lknAction.Text = "Documentos Cancelados";
		...
		this.lbText02.Text = "Neste relatório é possível controlar o total de dias entre a emissão dos documentos fiscais e a data de cancelamento.";
		...
		this.lbText01.Text = "Apresenta no painel os documentos que possuem evento de cancelamento registrados por seus emissores com o respectivo motivo.";
		...
		this.lbTitle01.Text = "Documentos Cancelados é ativado nos \r\nplanos Avançado e Enterprise.";
		...
		base.Name = "frmTabDocCanc";
		this.Text = "Documentos Cancelados";
		base.Load += new System.EventHandler(frmTabDocCanc_Load);
