...
public class frmTabExpIndRem : Form
{
	public class clsOlvIndRem : ICloneable
	{
		...
		public List<clsOlvIndRem> Children { get; set; } = new List<clsOlvIndRem>();

		public clsOlvIndRem GetClone()
		{
			return (clsOlvIndRem)MemberwiseClone();
		}
	...
	private clsSrvTabExpIndRem _SqlTabData = new clsSrvTabExpIndRem();
	...
	private ToolStripComboBox tscViewType;

	private ToolStripLabel tslViewType;
	...
	public frmTabExpIndRem(string pTitle, Color pColumnColor, string pFeatExtId)
	{
		InitializeComponent();
		Text = pTitle;
	...
	private async void frmTabExpIndRem_Load(object sender, EventArgs e)
	{
		_ = 9;
		try
		{
			...
			{
				clsScreenGeral.funcCheckListViewDdic(typeof(clsOlvIndRem), lsvData);
			}
			funcDefineListViewFeatures();
			lsvData.CellToolTipShowing += olv_CellToolTipShowing;
			List<clsObjectType> varclsObjList01 = new List<clsObjectType>();
			varclsObjList01.Add(new clsObjectType
			{
				Name = "Produtor",
				Value = "Producer"
			});
			...
			tscViewType.ComboBox.ValueMember = "Value";
			tscViewType.ComboBox.DisplayMember = "Name";
			tscViewType.ComboBox.DataSource = varclsObjList01;
			string varViewType = await _clsDataParam.funcGetAsync("TabExpIndRem-ViewType");
			if (clsFunction.IsEmpty(varViewType))
			{
				varViewType = "Producer";
			}
			tscViewType.ComboBox.SelectedValue = varViewType;
			List<clsObjectType> varclsObjList2 = new List<clsObjectType>();
			varclsObjList2.Add(new clsObjectType
			{
				Name = "Quantidade",
				Value = "Quantity"
			});
			...
			tscUnitType.ComboBox.DataSource = varclsObjList2;
			string varUnitType = await _clsDataParam.funcGetAsync("TabExpIndRem-UnitType");
	...
	{
		clsOlvIndRem varModel = (clsOlvIndRem)e.Model;
	...
	private void funcDefineListViewFeatures()
	{
		...
		{
			clsOlvIndRem clsOlvIndRem = (clsOlvIndRem)x;
			if (clsOlvIndRem == null)
			{
				return (object)null;
			}
			return (!clsFunction.IsEmpty(clsOlvIndRem.HasDossie)) ? ((object)28) : null;
		};
		olvDocNum.ImageGetter = delegate(object x)
		{
			...
			if (clsFunction.IsEqual(clsOlvIndRem.RegType, "REM-DOC-HEAD", "EXP-DOC-HEAD"))
			{
				if (clsFunction.IsEqual(clsOlvIndRem.Status, "SUCCESS"))
				{
					return 8;
				}
				if (clsFunction.IsEqual(clsOlvIndRem.Status, "WARNING"))
				{
					return 16;
				}
				if (clsFunction.IsEqual(clsOlvIndRem.Status, "ERROR"))
				{
					return 17;
				}
				return (object)null;
			}
			if (clsFunction.IsEqual(clsOlvIndRem.RegType, "REM-DOC-ITEM", "EXP-DOC-ITEM"))
			{
				if (clsFunction.IsEqual(clsOlvIndRem.Status, "SUCCESS"))
				{
					return 8;
		...
		lsvData.CanExpandGetter = delegate(object x)
		{
			...
			if (clsOlvIndRem.Children == null)
			{
				return false;
			}
			return clsOlvIndRem.Children.Count > 0;
		};
		lsvData.ChildrenGetter = (object x) => ((clsOlvIndRem)x)?.Children;
		olvTag.ImageGetter = delegate(object x)
		{
			...
			return clsFunction.IsEqual(clsOlvIndRem.RegType, "REM-DOC-HEAD", "EXP-DOC-HEAD") ? ((!clsFunction.IsEmpty(clsOlvIndRem.DocNote)) ? ((object)14) : ((object)(-1))) : ((object)(-1));
		};
		olvTag.AspectGetter = delegate(object x)
		{
			...
			return clsFunction.IsEqual(clsOlvIndRem.RegType, "REM-DOC-HEAD", "EXP-DOC-HEAD") ? (clsFunction.IsEmpty(clsOlvIndRem.Tag) ? null : _TagHsList[clsOlvIndRem.Tag]) : null;
		};
		olvTipoDoc.AspectGetter = delegate(object x)
		{
			clsOlvIndRem clsOlvIndRem = (clsOlvIndRem)x;
			return (clsOlvIndRem == null) ? null : clsComexCodes.funcGetTipoDocDesc(clsOlvIndRem.TipoDoc);
		};
		olvNatOper.AspectGetter = delegate(object x)
		{
			clsOlvIndRem clsOlvIndRem = (clsOlvIndRem)x;
			return (clsOlvIndRem == null) ? null : clsComexCodes.funcGetNatOperDesc(clsOlvIndRem.NatOper);
		};
		olvTpDocTrans.AspectGetter = delegate(object x)
		{
			clsOlvIndRem clsOlvIndRem = (clsOlvIndRem)x;
			return (clsOlvIndRem == null) ? null : clsComexCodes.funcGetTipoDocTransDesc(clsOlvIndRem.TpDocTrans);
		};
		olvViaTrans.AspectGetter = delegate(object x)
		{
			clsOlvIndRem clsOlvIndRem = (clsOlvIndRem)x;
			return (clsOlvIndRem == null) ? null : clsComexCodes.funcGetViaTransDesc(clsOlvIndRem.ViaTrans);
		};
		olvDueStatus.ImageGetter = delegate(object x)
		{
			...
			if (clsFunction.IsEmpty(clsOlvIndRem.DueStatus))
			{
				return (object)null;
			}
			if (clsFunction.Contains(clsOlvIndRem.DueStatus, "REGIST", pIgnoreCase: true))
			{
				return 23;
			}
			if (clsFunction.Contains(clsOlvIndRem.DueStatus, "PENDEN", pIgnoreCase: true))
			{
				return 24;
			}
			if (clsFunction.Contains(clsOlvIndRem.DueStatus, "AGUARD", pIgnoreCase: true))
			{
				return 24;
			}
			if (clsFunction.Contains(clsOlvIndRem.DueStatus, "PARCIAL", pIgnoreCase: true))
			{
				return 25;
			}
			if (clsFunction.Contains(clsOlvIndRem.DueStatus, "AVERBADA", pIgnoreCase: true))
			{
				return 26;
			}
			return clsFunction.Contains(clsOlvIndRem.DueStatus, "CANCEL", pIgnoreCase: true) ? ((object)27) : ((object)25);
		};
		olvDueCanal.ImageGetter = delegate(object x)
		{
			...
			if (clsFunction.Contains(clsOlvIndRem.DueCanal, "Verde", pIgnoreCase: true))
			{
				return 19;
			}
			if (clsFunction.Contains(clsOlvIndRem.DueCanal, "Amarelo", pIgnoreCase: true))
			{
				return 20;
			}
			if (clsFunction.Contains(clsOlvIndRem.DueCanal, "Laranja", pIgnoreCase: true))
			{
				return 21;
			}
			return clsFunction.Contains(clsOlvIndRem.DueCanal, "Vermelho", pIgnoreCase: true) ? ((object)22) : null;
		};
		olvLpcoStat.AspectGetter = delegate(object x)
		{
			...
			if (clsFunction.IsEmpty(clsOlvIndRem.LpcoStatus))
			{
				return string.Empty;
			}
			return _IsLpcoControlEnabled ? clsOlvIndRem.LpcoStatus : "Detalhes";
		};
	...
	public async Task<clsReturn> funcLoadDataAsync(clsDataFilter pclsDataFilter, bool pShow = true)
	{
		...
		try
		{
			...
			clsObjectType varViewType = (clsObjectType)tscViewType.SelectedItem;
			_SqlTabData.funcSetViewType(varViewType);
			...
			bool varByExport = clsFunction.IsEqual(varViewType.Value, "Exporter");
			...
			List<DueItemRem> varDueItemRemList = new List<DueItemRem>();
			List<DueItem> varDueItemList = new List<DueItem>();
			List<DueHeader> varDueHeaderList = new List<DueHeader>();
			List<Document> varDocExpListInDueItem = new List<Document>();
			List<DocItem> varDocExpItemListInDueItem = new List<DocItem>();
			if (varByExport)
			{
				varDueItemRemList = await _clsDataDueItemRem.funcGetListByRemNFeListAsync(varDocKeyList);
				List<string> varDueHeaderNumList = _clsDataDueItemRem.funcGetNumList(varDueItemRemList);
				varDueItemList = await _clsDataDueItem.funcGetListByNumListAsync(varDueHeaderNumList);
				List<string> varExpNFeKeyList = _clsDataDueItem.funcGetExpNFeKeyList(varDueItemList);
				varDocExpListInDueItem = await _clsDataDoc.funcGetListByKeyListAsync(varExpNFeKeyList);
				varDocExpItemListInDueItem = await _clsDataDocItem.funcGetListByChaveListAsync(varExpNFeKeyList);
				varDueHeaderList = await _clsDataDueHeader.funcGetItemByDueNumListAsync(varDueHeaderNumList, pNotCancel: true);
			}
			if (pShow)
			{
			...
			await Task.WhenAll(((IEnumerable<Document>)varclsDocList).Select((Func<Document, Task>)async delegate(Document varclsDoc)
			{
				try
				{
					...
					clsModelDoc.ComexDeadLine = await _clsDataComexDeadLine.funcGetAsync(varclsDoc, "IndRem", varStateList);
					...
					while (!varExitLoop)
					{
						...
						{
							int varPosAverbRem2 = varAverRemList.FindIndex((Event790700 r) => clsFunction.IsEqual(r.Chave, varclsModelItem2.DocItemRem.Chave) && clsFunction.IsEqualInt(r.nItem, varclsModelItem2.DocItemRem.nItem));
							bool varExitLoopAverb = varPosAverbRem2 < 0;
							while (!varExitLoopAverb)
							{
								try
								{
									Event790700 varAverbItem = varAverRemList[varPosAverbRem2];
									if (clsFunction.IsEqual(varAverbItem.Chave, varclsModelItem2.DocItemRem.Chave) && clsFunction.IsEqualInt(varAverbItem.nItem, varclsModelItem2.DocItemRem.nItem))
									{
										varclsModelItem2.AverbRemList.Add(varAverbItem);
										varclsModelItem2.AverbRemListCount++;
										varPosAverbRem2++;
										continue;
									}
								}
			...
			foreach (clsModelDoc varclsModelItem in varclsModeList)
			{
				...
				clsOlvIndRem varOlvRemHeader = new clsOlvIndRem();
				...
				{
					varOlvRemHeader.Descript = varclsDocRem.DestinNome;
					varOlvRemHeader.Identity = clsFunction.funcFormatDoc(varclsDocRem.DestinID);
				}
				...
				for (int varItemCount = 0; varItemCount < varclsModelItem.DocItemRemCount; varItemCount++)
				{
					...
					DocItem varDocItemRem = varDocItem.DocItemRem;
					clsOlvIndRem varOlvRemItem = new clsOlvIndRem();
					varOlvRemItem.RegType = "REM-DOC-ITEM";
					varOlvRemItem.Filial = varclsDocRem.Filial;
					varOlvRemItem.Chave = varclsDocRem.Chave;
					varOlvRemItem.DocNum = "Item " + clsFunction.funcGetValue(varDocItemRem.nItem).PadLeft(3, '0');
					varOlvRemItem.Descript = varDocItemRem.xProd;
					varOlvRemItem.ProdCode = varDocItemRem.cProd;
					varOlvRemItem.ProdNcm = varDocItemRem.NCM;
					varOlvRemItem.ProdCfop = varDocItemRem.CFOP;
					varOlvRemItem.QtRem = clsFunction.funcConvStrToDec(varDocItemRem.qTrib);
					varTotReme += varOlvRemItem.QtRem * (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
					if (!varByQuant)
					{
						varOlvRemItem.QtRem *= (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
					}
					if (!varByQuant)
					{
					...
					varOlvRemItem.Unit = varDocItemRem.uTrib;
					if (!varByQuant)
					{
					...
					if (varByExport)
					{
						for (int varDueItemRemCount = 0; varDueItemRemCount < varDocItem.DueItemRemCount; varDueItemRemCount++)
						{
							...
							clsOlvIndRem varOlvExptHeader = new clsOlvIndRem();
							...
							varOlvExptHeader.DtEmit = clsFunction.funcGetDate(varDocExport.DtEmi, pInCaseOfEmptyRetNull: true);
							...
							clsOlvIndRem varOlvExptItem = new clsOlvIndRem();
							...
							varTotExpt += varOlvExptItem.QtExport * (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
							if (!varByQuant)
							{
								varOlvExptItem.QtExport *= (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
							}
							if (!varByQuant)
							{
							...
							for (int varAverbCount = 0; varAverbCount < varDueItemModelRem.AverbRemListCount; varAverbCount++)
							{
								Event790700 varclsAverb01 = varDueItemModelRem.AverbRemList[varAverbCount];
								clsOlvIndRem varOlvAverbItem = new clsOlvIndRem();
								...
								varTotAver += varOlvAverbItem.QtAverb * (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
								if (!varByQuant)
								{
									varOlvAverbItem.QtAverb *= (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
								}
								...
								varOlvAverbItem.Unit = varDocItemRem.uTrib;
								if (!varByQuant)
								{
					...
					{
						for (int varAverbCount2 = 0; varAverbCount2 < varDocItem.AverbRemListCount; varAverbCount2++)
						{
							Event790700 varclsAverbItem = varDocItem.AverbRemList[varAverbCount2];
							clsOlvIndRem varOlvAverbItem2 = new clsOlvIndRem();
							varOlvAverbItem2.RegType = "AVERB-ITEM";
							varOlvAverbItem2.Filial = varclsDocRem.Filial;
							varOlvAverbItem2.Chave = varclsDocRem.Chave;
							varOlvAverbItem2.DocNum = "DUE " + varclsAverbItem.nDue + " Item " + varclsAverbItem.nItemDue;
							varOlvAverbItem2.Descript = varDocItemRem.xProd;
							varOlvAverbItem2.ProdCode = varDocItemRem.cProd;
							varOlvAverbItem2.ProdNcm = varDocItemRem.NCM;
							varOlvAverbItem2.ProdCfop = varDocItemRem.CFOP;
							varOlvAverbItem2.QtAverb = clsFunction.funcConvStrToDec(varclsAverbItem.qItem);
							varTotAver += varOlvAverbItem2.QtAverb * (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
							varTotExpt += varOlvAverbItem2.QtAverb * (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
							if (!varByQuant)
							{
								varOlvAverbItem2.QtAverb *= (decimal?)clsFunction.funcConvStrToDec(varDocItemRem.vUnTrib);
							}
							varOlvAverbItem2.QtExport = varOlvAverbItem2.QtAverb;
							varOlvRemItem.QtAverb += varOlvAverbItem2.QtAverb;
							if (!varByQuant)
							{
								varOlvRemHeader.QtAverb += varOlvAverbItem2.QtAverb;
							}
							varOlvRemItem.QtExport += varOlvAverbItem2.QtAverb;
							if (!varByQuant)
							{
								varOlvRemHeader.QtExport += varOlvAverbItem2.QtAverb;
							}
							varOlvAverbItem2.Unit = varDocItemRem.uTrib;
							if (!varByQuant)
							{
								varOlvAverbItem2.Unit = "R$";
							}
							varOlvAverbItem2.DtEmbar = clsFunction.funcGetDate(varclsAverbItem.dtEmbarque);
							varOlvAverbItem2.DataAverb = clsFunction.funcGetDate(varclsAverbItem.dtAverbacao);
							varOlvAverbItem2.DueItem = varclsAverbItem.nItemDue;
							varOlvRemItem.Children.Add(varOlvAverbItem2);
						}
					}
	...
	{
		List<clsOlvIndRem> varObjList = funcGetObjList(pFocused: true, pChecked: true);
	...
	public async Task<bool> funcSetTagAsync(string pTagCode, bool pFocused, bool pChecked)
	{
		...
		List<clsOlvIndRem> varObjList = funcGetObjList(pFocused, pChecked);
		List<Document> varDocList = await funcGetDocListAsync(varObjList, pSyncFromDbaFirst: true);
		if (varDocList == null)
		{
			return false;
	...
	public void funcSearchText(string pSearchTerm, bool pSearchInteligent)
	{
		...
		else
		{
			if (pSearchInteligent)
			{
				...
				{
					if (clsFunction.IsEqual(column.Text == "Número", true))
					{
	...
	public async Task<clsReturn> funcExportToExcelAsync()
	{
		...
		try
		{
			...
			plnMessage.Visible = true;
			Application.DoEvents();
			int varBlankCols = (clsFunction.IsEqual(((clsObjectType)tscViewType.SelectedItem).Value, "Exporter") ? 5 : 2);
			varclsReturnFunc = await clsScreenGeral.funcExportDocTreeToExcelAsync<clsOlvIndRem>(lsvData, varBlankCols);
		}
		catch (Exception pException)
		{
			varclsReturnFunc.AddException(pException);
	...
	public async Task<List<Document>> funcGetDocListAsync(List<clsOlvIndRem> pObjList, bool pSyncFromDbaFirst)
	{
		List<Document> varDocList = new List<Document>();
		foreach (clsOlvIndRem varObject in pObjList)
		{
	...
	public List<clsOlvIndRem> funcGetObjList(bool pFocused, bool pChecked, bool pOnlyPend = false)
	{
		List<clsOlvIndRem> varObjList = new List<clsOlvIndRem>();
		clsOlvIndRem varFocused = new clsOlvIndRem();
		if (pFocused && lsvData.FocusedItem != null)
		{
			try
			{
				varFocused = (clsOlvIndRem)lsvData.FocusedObject;
				varObjList.Add(varFocused);
			}
			catch
			{
				varFocused = new clsOlvIndRem();
			}
		}
		if (pChecked)
		{
			foreach (clsOlvIndRem varObject in lsvData.SelectedObjects)
			{
			...
			{
				varObjList = lsvData.Objects.Cast<clsOlvIndRem>().Intersect(varObjList).ToList();
			}
		}
		if ((!pFocused && !pChecked) || varObjList.Count <= 0)
		{
			varObjList = lsvData.FilteredObjects.Cast<clsOlvIndRem>().ToList();
			if (varObjList.Count <= 0)
			{
				varObjList = lsvData.Objects.Cast<clsOlvIndRem>().ToList();
			}
		}
		if (pOnlyPend && !clsFunction.IsAdmin)
		{
			varObjList.RemoveAll((clsOlvIndRem r) => clsFunction.Contains(r.Status, "SUCCESS", "AVERBADA", pIgnoreCase: true));
		}
		return varObjList;
	}
	...
	private async void funcRefreshListAsync(List<Document> pDocList, List<clsOlvIndRem> pObjList)
	{
		ConcurrentBag<ObjFieldBuffer> varDocFieldList = clsObjectBuffer.funcGet<Document>().Fields;
		ConcurrentBag<ObjFieldBuffer> varObjFieldList = clsObjectBuffer.funcGet<clsOlvIndRem>().Fields;
		await Task.WhenAll(((IEnumerable<clsOlvIndRem>)pObjList).Select((Func<clsOlvIndRem, Task>)async delegate(clsOlvIndRem varclsObjItem)
		{
	...
	private void lsvData_FormatCell(object sender, FormatCellEventArgs e)
	{
		...
		{
			clsOlvIndRem varDocument = (clsOlvIndRem)e.Model;
	...
	{
		clsOlvIndRem varclsObject = (clsOlvIndRem)e.Model;
	...
	private async void funcShowDocViewerAsync(clsOlvIndRem pclsObject, bool pShowPDF, bool pShowXML)
	{
	...
	private void funcRebuildFilters()
	{
		...
		{
			varFilters.Add(new ModelFilter((object model) => clsFunction.IsEqual(((clsOlvIndRem)model).Status, "SUCCESS", "WARNING", "ERROR", "") || !clsFunction.IsEqual(((clsOlvIndRem)model).RegType, "REM-DOC-HEAD")));
		}
		else if (tsbSuccess.Checked && tsbWarning.Checked)
		{
			varFilters.Add(new ModelFilter((object model) => clsFunction.IsEqual(((clsOlvIndRem)model).Status, "SUCCESS", "WARNING", "") || !clsFunction.IsEqual(((clsOlvIndRem)model).RegType, "REM-DOC-HEAD")));
		}
		else if (tsbWarning.Checked && tsbError.Checked)
		{
			varFilters.Add(new ModelFilter((object model) => clsFunction.IsEqual(((clsOlvIndRem)model).Status, "WARNING", "ERROR", "") || !clsFunction.IsEqual(((clsOlvIndRem)model).RegType, "REM-DOC-HEAD")));
		}
		else if (tsbSuccess.Checked && tsbError.Checked)
		{
			varFilters.Add(new ModelFilter((object model) => clsFunction.IsEqual(((clsOlvIndRem)model).Status, "SUCCESS", "ERROR", "") || !clsFunction.IsEqual(((clsOlvIndRem)model).RegType, "REM-DOC-HEAD")));
		}
		else if (tsbError.Checked)
		{
			varFilters.Add(new ModelFilter((object model) => clsFunction.IsEqual(((clsOlvIndRem)model).Status, "ERROR", "") || !clsFunction.IsEqual(((clsOlvIndRem)model).RegType, "REM-DOC-HEAD")));
		}
		else if (tsbWarning.Checked)
		{
			varFilters.Add(new ModelFilter((object model) => clsFunction.IsEqual(((clsOlvIndRem)model).Status, "WARNING", "") || !clsFunction.IsEqual(((clsOlvIndRem)model).RegType, "REM-DOC-HEAD")));
		}
		else if (tsbSuccess.Checked)
		{
			varFilters.Add(new ModelFilter((object model) => clsFunction.IsEqual(((clsOlvIndRem)model).Status, "SUCCESS", "") || !clsFunction.IsEqual(((clsOlvIndRem)model).RegType, "REM-DOC-HEAD")));
		}
	...
	private async void tsbSync_Click(object sender, EventArgs e)
	{
		...
		try
		{
			...
			plnMessage.Visible = true;
			Application.DoEvents();
			varclsReturnFunc = ((!clsFunction.IsEqual(((clsObjectType)tscViewType.SelectedItem).Value, "Exporter")) ? (await funcGetComexProducerAsync()) : (await funcGetComexExporterAsync()));
	...
	private async Task<clsReturn> funcGetComexExporterAsync()
	{
		clsReturn varclsReturnFunc = new clsReturn();
		...
		try
		{
			...
			Application.DoEvents();
			List<clsOlvIndRem> varObjectList = funcGetObjList(pFocused: false, pChecked: true, pOnlyPend: true);
			...
			foreach (clsOlvIndRem varclsItem in varObjectList)
			{
				varCounter01++;
	...
	private async Task<clsReturn> funcGetComexProducerAsync()
	{
		clsReturn varclsReturnFunc = new clsReturn();
		clsComexApi varclsComexApi = new clsComexApi();
		try
		{
			...
			List<clsOlvIndRem> varObjectList = funcGetObjList(pFocused: false, pChecked: true, pOnlyPend: true);
			int varCounter = 0;
			int varTotal = varObjectList.Count;
			FilialView varclsFilial = new FilialView();
			foreach (clsOlvIndRem varclsItem in varObjectList)
			{
				varCounter++;
				...
				lbProgress.Text = $"[ {varCounter} de {varTotal} ] Sincronizado dados da NFe {varclsItem.DocNum}...";
				Application.DoEvents();
				clsReturn varclsRetItem = await varclsComexApi.funcGetDataByNFeDueListScanAsync(varclsFilial, varclsItem.Chave, pByTaskAction: false);
	...
	private async void tscUnitType_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		await _clsDataParam.funcSetAsync("TabExpIndRem-UnitType", varSelected);
		try
		{
	...
	private async void tscViewType_SelectedIndexChanged(object sender, EventArgs e)
	{
		if (_IsLoading)
		{
			return;
		}
		bool varByExport = clsFunction.IsEqual(((clsObjectType)tscViewType.SelectedItem).Value, "Exporter");
		ToolStripButton toolStripButton = tsbDocs;
		bool visible = (tssSepDocs.Visible = varByExport);
		toolStripButton.Visible = visible;
		string varSelected = clsFunction.funcGetValue(((clsObjectType)tscViewType.SelectedItem).Value);
		await _clsDataParam.funcSetAsync("TabExpIndRem-ViewType", varSelected);
		try
		{
			lbProgress.Text = "Recarregando os dados...";
			plnMessage.Visible = true;
	...
	{
		clsHelpService.funcCallTipReportExportIndirectAsync();
	}
	...
	private void funcSetTabDataTotalValues()
	{
		...
		List<clsOlvIndRem> varObjList = varEnumList.Cast<clsOlvIndRem>().ToList();
		if (varObjList == null)
		{
			varObjList = new List<clsOlvIndRem>();
		}
		_TotalValue = varObjList.Sum((clsOlvIndRem r) => clsFunction.funcConvStrToDec(r.Valor));
	...
	private clsComexApi.clsDueObject funcGetDueObjItem(clsOlvIndRem pclsObject)
	{
		...
		foreach (clsOlvIndRem varclsItem in pclsObject.Children)
		{
	...
	{
		List<clsOlvIndRem> list = funcGetObjList(pFocused: true, pChecked: true);
		List<clsComexApi.clsDueObject> varDueObjList = new List<clsComexApi.clsDueObject>();
		foreach (clsOlvIndRem varclsItem in list)
		{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Monitor.frmTabExpIndRem));
		...
		this.tslViewType = new System.Windows.Forms.ToolStripLabel();
		this.tscViewType = new System.Windows.Forms.ToolStripComboBox();
		...
		this.olvDueStatus.Tag = "Exporter";
		...
		this.olvPaisDestName.Tag = "Exporter";
		...
		this.olvConEmbNum.Tag = "Exporter";
		...
		this.olvConEmbTipo.Tag = "Exporter";
		...
		this.olvConEmbData.Tag = "Exporter";
		...
		this.olvTpDocTrans.Tag = "Exporter";
		...
		this.tspTaskMenu.Items.AddRange(new System.Windows.Forms.ToolStripItem[17]
		{
			this.toolStripSeparator3, this.tsbSuccess, this.toolStripSeparator6, this.tsbWarning, this.toolStripSeparator4, this.tsbError, this.toolStripSeparator5, this.tslViewType, this.tscViewType, this.toolStripSeparator7,
			this.tslUnitType, this.tscUnitType, this.toolStripSeparator2, this.tsbSync, this.tssSepComex, this.tsbDocs, this.tssSepDocs
		});
		...
		this.tslViewType.Name = "tslViewType";
		this.tslViewType.Size = new System.Drawing.Size(39, 20);
		this.tslViewType.Text = "Visão :";
		this.tscViewType.BackColor = System.Drawing.SystemColors.Info;
		this.tscViewType.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
		this.tscViewType.Font = new System.Drawing.Font("Tahoma", 8.25f);
		this.tscViewType.Name = "tscViewType";
		this.tscViewType.Size = new System.Drawing.Size(130, 23);
		this.tscViewType.SelectedIndexChanged += new System.EventHandler(tscViewType_SelectedIndexChanged);
		...
		this.lknAction.Text = "Exportação Indireta: saldos e Prazos";
		...
		this.lbTitle01.Text = "Exportação Indireta: Saldos e Prazos é ativado nos \r\nplanos Avançado e Enterprise.";
		...
		base.Name = "frmTabExpIndRem";
		this.Text = "Exportação Indireta: Saldos e Prazos";
		base.Load += new System.EventHandler(frmTabExpIndRem_Load);
