...
public class frmTabDocNFePendCTe : Form
{
	...
	private clsSrvTabDocNFePendCTe _SqlTabData = new clsSrvTabDocNFePendCTe();
	...
	private ListView lsvTabDocNFePendCTe;
	...
	public frmTabDocNFePendCTe(string pTitle, Color pColumnColor, string pFeatExtId)
	{
		InitializeComponent();
		Text = pTitle;
	...
	private async void frmTabDocNFePendCTe_Load(object sender, EventArgs e)
	{
		...
		clsFunction.funcSetColumnsConfiguration(this, lsvTabDocNFePendCTe);
		_ColumnsLoaded = true;
		lsvTabDocNFePendCTe.Columns[0].ImageIndex = 16;
		...
		if (_IsLocked)
		{
			...
			lsvTabDocNFePendCTe.Dock = DockStyle.None;
		...
		{
			lsvTabDocNFePendCTe.Dock = DockStyle.Fill;
	...
	private async Task<clsReturn> funcShowDataAsync(clsDataFilter pclsFilter, List<Document> pDocList)
	{
		...
		lsvTabDocNFePendCTe.BeginUpdate();
		lsvTabDocNFePendCTe.SuspendLayout();
		lsvTabDocNFePendCTe.Items.Clear();
		int varCounter = 0;
		...
		{
			lsvTabDocNFePendCTe.ShowGroups = false;
			lsvTabDocNFePendCTe.Groups.Clear();
		}
		else
		{
			lsvTabDocNFePendCTe.ShowGroups = true;
		}
		foreach (Document varclsDocument in pDocList)
		{
			...
			if (pclsFilter.GroupByPartner)
			{
				...
				if (!varGroupByText.Equals(varPartnerIdnt))
				{
					...
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varPartnerIdnt, varGroupHeaderText);
				}
			...
			else if (pclsFilter.GroupByDate)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varclsDocument.DtAut, varclsDocument.DtAut);
				}
			...
			else if (pclsFilter.GroupByYearMonth)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varAnoMes, varAnoMes);
				}
			...
			else if (pclsFilter.GroupByState)
			{
				if (!varGroupByText.Equals(varclsDocument.cUF))
				{
					...
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varclsDocument.cUF, varGroupHeaderText2);
				}
			...
			else if (pclsFilter.GroupByModel)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varModel, varModel);
				}
			...
			else if (pclsFilter.GroupByTpDoc)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varTipoDoc, varTipoDoc);
				}
			...
			else if (pclsFilter.GroupByNatOper)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varNatOper, varNatOper);
				}
			...
			else if (pclsFilter.GroupByCFOP)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varDocCFOP, varDocCFOP);
				}
			...
			else if (pclsFilter.GroupByFilial)
			{
				...
				if (!varGroupByText.Equals(varFilialId))
				{
					...
					varListGroup = lsvTabDocNFePendCTe.Groups.Add(varFilialId, varGroupHeaderText3);
				}
			...
			lsvTabDocNFePendCTe.Items.Add(varListItem);
		}
		lsvTabDocNFePendCTe.EndUpdate();
		lsvTabDocNFePendCTe.ResumeLayout();
	...
	{
		if (lsvTabDocNFePendCTe.FocusedItem == null)
		{
			return false;
		}
		ObjDoc varObjDoc = (ObjDoc)lsvTabDocNFePendCTe.FocusedItem.Tag;
		if (varObjDoc == null)
		{
			return false;
		}
	...
	{
		foreach (ListViewItem varListItem in lsvTabDocNFePendCTe.Items)
		{
	...
	{
		return await clsScreenGeral.funcExportToCsvAsync(lsvTabDocNFePendCTe, onProgressChange);
	}

	private List<ListViewItem> funcGetObjList(bool pFocused, bool pChecked)
	{
		return clsFunction.funcGetListViewItens(lsvTabDocNFePendCTe, pAllItens: false, pFocused, pChecked);
	}
	...
	{
		return lsvTabDocNFePendCTe.Items.Count;
	}
	...
	{
		if (lsvTabDocNFePendCTe != null)
		{
			IList varObjectsOnView = lsvTabDocNFePendCTe.CheckedItems;
			if (varObjectsOnView.Count <= 0)
			{
				varObjectsOnView = lsvTabDocNFePendCTe.Items;
			}
	...
	{
		clsHelpService.funcCallTipReportNFePendTransportAsync();
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Monitor.frmTabDocNFePendCTe));
		...
		this.lsvTabDocNFePendCTe = new System.Windows.Forms.ListView();
		...
		this.pnContent.Controls.Add(this.lsvTabDocNFePendCTe);
		...
		this.lsvTabDocNFePendCTe.AllowColumnReorder = true;
		this.lsvTabDocNFePendCTe.Anchor = System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Left | System.Windows.Forms.AnchorStyles.Right;
		this.lsvTabDocNFePendCTe.CheckBoxes = true;
		this.lsvTabDocNFePendCTe.Columns.AddRange(new System.Windows.Forms.ColumnHeader[18]
		{
		...
		this.lsvTabDocNFePendCTe.ContextMenuStrip = this.contextMenuDocs;
		this.lsvTabDocNFePendCTe.Font = new System.Drawing.Font("Verdana", 8.25f, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, 0);
		this.lsvTabDocNFePendCTe.FullRowSelect = true;
		this.lsvTabDocNFePendCTe.HideSelection = false;
		this.lsvTabDocNFePendCTe.Location = new System.Drawing.Point(0, 0);
		this.lsvTabDocNFePendCTe.Name = "lsvTabDocNFePendCTe";
		this.lsvTabDocNFePendCTe.ShowItemToolTips = true;
		this.lsvTabDocNFePendCTe.Size = new System.Drawing.Size(784, 116);
		this.lsvTabDocNFePendCTe.SmallImageList = this.ImageListDocs;
		this.lsvTabDocNFePendCTe.TabIndex = 8;
		this.lsvTabDocNFePendCTe.UseCompatibleStateImageBehavior = false;
		this.lsvTabDocNFePendCTe.View = System.Windows.Forms.View.Details;
		this.lsvTabDocNFePendCTe.ColumnClick += new System.Windows.Forms.ColumnClickEventHandler(lstView_ColumnClick);
		this.lsvTabDocNFePendCTe.ColumnReordered += new System.Windows.Forms.ColumnReorderedEventHandler(lstView_ColumnReordered);
		this.lsvTabDocNFePendCTe.ColumnWidthChanged += new System.Windows.Forms.ColumnWidthChangedEventHandler(lstView_ColumnWidthChanged);
		this.lsvTabDocNFePendCTe.ItemChecked += new System.Windows.Forms.ItemCheckedEventHandler(lstView_ItemChecked);
		this.lsvTabDocNFePendCTe.DoubleClick += new System.EventHandler(lstView_DoubleClick);
		...
		this.lknAction.Text = "NFe Pendentes de Transporte";
		...
		this.lbText01.Text = "Lista todas as NFe de entrada e de saída que possuem uma transportadora destacada como responsável pelo frete a qual ainda não emitiu o CTe ou MDFe devido.";
		...
		this.lbTitle01.Text = "NFes Pendentes de Transporte é ativado nos \r\nplanos Avançado e Enterprise.";
		...
		base.Name = "frmTabDocNFePendCTe";
		this.Text = "NFes Pendentes de Transporte";
		base.Load += new System.EventHandler(frmTabDocNFePendCTe_Load);
