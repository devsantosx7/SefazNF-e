...
[ReportClass]
public class clsTabExpDueControl : clsAbsTabData, intDocTabData, IDisposable
{
	private frmTabExpDueControl _FormTabData;

	private clsSrvTabDocIn _SqlTabData = new clsSrvTabDocIn();

	public clsTabExpDueControl()
	{
		base.TabName = "TabExpDueControl";
		base.TabText = "Controle de Exportação";
		base.FeatExtId = clsFeatureService.consReportExpDueControl;
		base.TabColor = Color.FromArgb(225, 248, 194);
		_FormTabData = new frmTabExpDueControl(base.TabText, base.TabColor, base.FeatExtId);
	...
	public async Task<clsReturn> funcLoadDataAsync(clsDataFilter pclsDataFilter)
	{
		...
		return await _FormTabData.funcLoadDataAsync(pclsDataFilter);
	}
