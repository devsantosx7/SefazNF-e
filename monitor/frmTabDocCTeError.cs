...
public class frmTabDocCTeError : Form
{
	private clsSrvTabDocCTeError _SqlTabData = new clsSrvTabDocCTeError();
	...
	public frmTabDocCTeError(string pTitle, Color pColumnColor, string pFeatExtId)
	{
		InitializeComponent();
		Text = pTitle;
	...
	private async void frmTabDocCTeError_Load(object sender, EventArgs e)
	{
		...
		{
			clsScreenGeral.funcCheckListViewDdic(typeof(DocCTeError), lsvData);
		}
	...
	private void funcDefineListViewFeatures()
	{
		...
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return clsFunction.IsEmpty(docCTeError?.Valor) ? null : ((object)clsFunction.funcConvStrToDec(docCTeError.Valor));
		};
		olvHasXml.ImageGetter = delegate(object x)
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : ((object)clsScreenGeral.funcGetDocXmlIcon(docCTeError, _HasNFSeFeatEnabled, _HasCFeFeatEnabled));
		};
		...
		olvEmitida.AspectGetter = delegate(object x)
		{
			...
			return clsFunction.IsEmpty(docCTeError.Emitida) ? "Terceiros" : "Empresa";
		};
		olvTag.ImageGetter = delegate(object x)
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : ((object)clsScreenGeral.funcGetDocNotesIcon(docCTeError));
		};
		olvTag.AspectGetter = delegate(object x)
		{
			...
			return clsFunction.IsEmpty(docCTeError.Tag) ? null : _TagHsList[docCTeError.Tag];
		};
		olvNum.AspectGetter = delegate(object x)
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : clsFunction.funcGetValue(docCTeError.Num).PadLeft(9, '0');
		};
		olvEstado.AspectGetter = delegate(object x)
		{
			DocCTeError varDocument = (DocCTeError)x;
		...
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : clsFunction.funcGetDocType(docCTeError.Model);
		};
		...
		olvStatus.AspectGetter = delegate(object x)
		{
			new List<int>();
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : clsScreenGeral.funcGetDocStatIcon(docCTeError);
		};
		olvManifest.AspectGetter = delegate(object x)
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : _clsDFeCodes.GetEventDesc(docCTeError.Model, docCTeError.ManifCode);
		};
		olvCNPJ.AspectGetter = delegate(object x)
		{
			...
			if (string.IsNullOrEmpty(docCTeError.Emitida))
			{
				return docCTeError.EmitID;
			}
			if (!string.IsNullOrEmpty(docCTeError.TomaID))
			{
				return docCTeError.TomaID;
			}
			return (!string.IsNullOrEmpty(docCTeError.DestinID)) ? docCTeError.DestinID : docCTeError.RecebeID;
		};
		olvNome.AspectGetter = delegate(object x)
		{
			...
			if (string.IsNullOrEmpty(docCTeError.Emitida))
			{
				return docCTeError.EmitNome;
			}
			if (!string.IsNullOrEmpty(docCTeError.TomaNome))
			{
				return docCTeError.TomaNome;
			}
			return (!string.IsNullOrEmpty(docCTeError.DestinNome)) ? docCTeError.DestinNome : docCTeError.RecebeNome;
		};
		olvFilial.AspectGetter = delegate(object x)
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : clsFunction.funcFormatDoc(docCTeError.Filial);
		};
		olvManifest.AspectGetter = delegate(object x)
		{
			DocCTeError docCTeError = (DocCTeError)x;
		...
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : _clsDFeCodes.funcGetTipoDoc(docCTeError.Model, docCTeError.TpDoc);
		};
		olvAnoMes.AspectGetter = delegate(object x)
		{
			...
			try
			{
				result = docCTeError.DtAut.Substring(0, 7);
			}
			catch
			{
			}
		...
		{
			DocCTeError docCTeError = (DocCTeError)x;
			return (docCTeError == null) ? null : varclsValidator.funcGetDesc(docCTeError);
		};
		olvReason.AspectGetter = delegate(object x)
		{
			...
			if (clsFunction.IsEqual(docCTeError.Reason, "Referência de incorreta de NF Modelo A1"))
			{
				return "Referência incorreta de NF Modelo A1";
			}
			if (clsFunction.IsEqual(docCTeError.Reason, "Transporte Pro\u00b4prio por conta do Remetente mas CTe emitido por Terceiros"))
			{
				return "Transporte Próprio por conta do Remetente, mas CTe emitido por Terceiros";
			}
			return clsFunction.IsEqual(docCTeError.Reason, "Transporte Pro\u00b4prio por conta do Destinatário mas CTe emitido por Terceiros") ? "Transporte Próprio por conta do Destinatário, mas CTe emitido por Terceiros" : docCTeError.Reason;
		};
	}
	...
	public async Task<clsReturn> funcLoadDataAsync(clsDataFilter pclsDataFilter)
	{
		...
		List<DocCTeError> varclsDocList = await new clsDataDocCTeError().funcGetListByFullSqlAsync(varSqlQuery, varPageSize);
	...
	{
		DocCTeError varDocCTeError = (DocCTeError)e.Model;
		if (varDocCTeError != null)
		{
			_ = string.Empty;
			if (e.ColumnIndex.Equals(olvHasXml.Index))
			{
				e.Text = varclsValidator.funcGetDesc(varDocCTeError);
			}
			else if (e.ColumnIndex.Equals(olvStatus.Index))
			{
				e.Text = clsScreenGeral.funcGetDocStatDesc(varDocCTeError);
			}
			else if (e.ColumnIndex.Equals(olvTag.Index))
			{
				e.Text = varDocCTeError.DocNote;
			}
		}
	}
	...
	public async Task<bool> funcSetTagAsync(string pTagCode, bool pFocused, bool pChecked)
	{
		...
		List<DocCTeError> varObjList = funcGetObjList(pFocused, pChecked);
	...
	{
		List<DocCTeError> varObjList = funcGetObjList(pFocused: true, pChecked: true);
		List<Document> varDocList = await funcGetDocListAsync(varObjList, pSyncFromDbaFirst: true);
		if (varDocList == null)
		{
			return false;
	...
	{
		return await clsScreenGeral.funcExportDocToExcelAsync<DocCTeError>(lsvData, onProgressChange);
	}
	...
	private async Task<List<Document>> funcGetDocListAsync(List<DocCTeError> pObjList, bool pSyncFromDbaFirst)
	{
		List<Document> varDocList = new List<Document>();
		foreach (DocCTeError varObject in pObjList)
		{
	...
	private List<DocCTeError> funcGetObjList(bool pFocused, bool pChecked)
	{
		List<DocCTeError> varDocList = new List<DocCTeError>();
		DocCTeError varFocused = new DocCTeError();
		if (pFocused && lsvData.FocusedItem != null)
		{
			try
			{
				varFocused = (DocCTeError)lsvData.GetItem(lsvData.FocusedItem.Index).RowObject;
				varDocList.Add(varFocused);
			}
			catch
			{
				varFocused = new DocCTeError();
			}
		}
		if (pChecked)
		{
			foreach (DocCTeError varObject in lsvData.CheckedObjects)
			{
			...
			if (varDocList.Count > 0)
			{
				varDocList = lsvData.Objects.Cast<DocCTeError>().Intersect(varDocList).ToList();
			}
		}
		if (!pFocused && !pChecked)
		{
			varDocList = lsvData.FilteredObjects.Cast<DocCTeError>().ToList();
			if (varDocList.Count <= 0)
			{
				varDocList = lsvData.Objects.Cast<DocCTeError>().ToList();
			}
		}
		return varDocList;
	}
	...
	private async void funcRefreshListAsync(List<Document> pDocList, List<DocCTeError> pObjList)
	{
		ConcurrentBag<ObjFieldBuffer> varDocFieldList = clsObjectBuffer.funcGet<Document>().Fields;
		ConcurrentBag<ObjFieldBuffer> varObjFieldList = clsObjectBuffer.funcGet<DocCTeError>().Fields;
		await Task.WhenAll(((IEnumerable<DocCTeError>)pObjList).Select((Func<DocCTeError, Task>)async delegate(DocCTeError varclsObjItem)
		{
	...
	private async void funcShowDocViewerAsync(bool pShowPDF, bool pShowXML, DocCTeError pDocCTeError = null)
	{
		new List<Document>();
		List<Document> varDocList;
		if (pDocCTeError != null)
		{
			List<DocCTeError> varObjList = new List<DocCTeError> { pDocCTeError };
	...
	private void lsvData_FormatCell(object sender, FormatCellEventArgs e)
	{
		...
		{
			DocCTeError varDocument = (DocCTeError)e.Model;
	...
	private void lsvData_HyperlinkClicked(object sender, HyperlinkClickedEventArgs e)
	{
		if (e.ColumnIndex == olvNum.Index)
		{
			...
			if (varDocCTeError != null)
			{
				funcShowDocViewerAsync(pShowPDF: true, pShowXML: false, varDocCTeError);
			}
		}
	}
	...
	private async Task<bool> funcTransferFilialAsync()
	{
		...
		varArguments.TaskProgress = "Transferindo DocCTeErroro(s) para o destino...";
		...
		{
			if (clsFunction.IsEqual(varclsValue.Name, "DocCTeError", pIgnoreCase: true))
			{
	...
	private void funcSetTabDataTotalValues()
	{
		if (!_IsOnHeaderCheckStatus)
		{
			...
			List<DocCTeError> varObjList = varEnumList.Cast<DocCTeError>().ToList();
			if (varObjList == null)
			{
				varObjList = new List<DocCTeError>();
			}
			_TotalValue = varObjList.Sum((DocCTeError r) => clsFunction.funcConvStrToDec(r.Valor));
			EventTabManagerEventArgs varArguments = new EventTabManagerEventArgs();
	...
	{
		clsHelpService.funcCallTipReportCTeIncorretAsync();
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Monitor.frmTabDocCTeError));
		...
		this.lsvData.Columns.AddRange(new System.Windows.Forms.ColumnHeader[23]
		{
			...
			this.olvManifest, this.olvCancel, this.olvReason, this.olvCNPJ, this.olvNome, this.olvChave, this.olvFilial, this.olvCFOP, this.olvError, this.olvDocNote,
			this.olvTpDoc, this.olvAnoMes, this.olvEstado
		});
		...
		this.olvReason.Text = "Resultado da análise";
		...
		this.lknAction.Text = "CTe incorretos";
		...
		this.lbText01.Text = "Lista todos os CTe que o sistema identifica como incorretos, seguindo dois critérios: citação correta da NFe e correspondência da modalidade de frete entre CTe e NFe.";
		...
		this.lbTitle01.Text = "CTes incorretos é ativado nos \r\nplanos Avançado e Enterprise.";
		...
		base.Name = "frmTabDocCTeError";
		this.Text = "CTes incorretos";
		base.Load += new System.EventHandler(frmTabDocCTeError_Load);
