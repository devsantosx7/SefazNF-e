...
public class frmTabDocRetPropr : Form
{
	...
	private clsSrvTabDocRetPropr _SqlTabData = new clsSrvTabDocRetPropr();
	...
	private ListView lsvTabDocRetPropr;
	...
	public frmTabDocRetPropr(string pTitle, Color pColumnColor, string pFeatExtId)
	{
		InitializeComponent();
		Text = pTitle;
	...
	private async void frmTabDocRetPropr_Load(object sender, EventArgs e)
	{
		...
		clsFunction.funcSetColumnsConfiguration(this, lsvTabDocRetPropr);
		_ColumnsLoaded = true;
		lsvTabDocRetPropr.Columns[0].ImageIndex = 16;
		...
		if (_IsLocked)
		{
			...
			lsvTabDocRetPropr.Dock = DockStyle.None;
		...
		{
			lsvTabDocRetPropr.Dock = DockStyle.Fill;
	...
	private async Task<clsReturn> funcShowDataAsync(clsDataFilter pclsFilter, List<Document> pDocList)
	{
		...
		lsvTabDocRetPropr.BeginUpdate();
		lsvTabDocRetPropr.SuspendLayout();
		lsvTabDocRetPropr.Items.Clear();
		int varCounter = 0;
		...
		{
			lsvTabDocRetPropr.ShowGroups = false;
			lsvTabDocRetPropr.Groups.Clear();
		}
		else
		{
			lsvTabDocRetPropr.ShowGroups = true;
		}
		foreach (Document varclsDocument in pDocList)
		{
			...
			if (pclsFilter.GroupByPartner)
			{
				...
				if (!varGroupByText.Equals(varPartnerIdnt))
				{
					...
					varListGroup = lsvTabDocRetPropr.Groups.Add(varPartnerIdnt, varGroupHeaderText);
				}
			...
			else if (pclsFilter.GroupByDate)
			{
				...
				{
					varListGroup = lsvTabDocRetPropr.Groups.Add(varclsDocument.DtAut, varclsDocument.DtAut);
				}
			...
			else if (pclsFilter.GroupByYearMonth)
			{
				...
				{
					varListGroup = lsvTabDocRetPropr.Groups.Add(varAnoMes, varAnoMes);
				}
			...
			else if (pclsFilter.GroupByState)
			{
				if (!varGroupByText.Equals(varclsDocument.cUF))
				{
					...
					varListGroup = lsvTabDocRetPropr.Groups.Add(varclsDocument.cUF, varGroupHeaderText2);
				}
			...
			else if (pclsFilter.GroupByModel)
			{
				...
				{
					varListGroup = lsvTabDocRetPropr.Groups.Add(varModel, varModel);
				}
			...
			else if (pclsFilter.GroupByTpDoc)
			{
				...
				{
					varListGroup = lsvTabDocRetPropr.Groups.Add(varTipoDoc, varTipoDoc);
				}
			...
			else if (pclsFilter.GroupByNatOper)
			{
				...
				{
					varListGroup = lsvTabDocRetPropr.Groups.Add(varNatOper, varNatOper);
				}
			...
			else if (pclsFilter.GroupByCFOP)
			{
				...
				{
					varListGroup = lsvTabDocRetPropr.Groups.Add(varDocCFOP, varDocCFOP);
				}
			...
			else if (pclsFilter.GroupByFilial)
			{
				...
				if (!varGroupByText.Equals(varFilialId))
				{
					...
					varListGroup = lsvTabDocRetPropr.Groups.Add(varFilialId, varGroupHeaderText3);
				}
			...
			else if (pclsFilter.GroupByTomaIE)
			{
				...
				{
					varListGroup = lsvTabDocRetPropr.Groups.Add(varDocTomaIE, varDocTomaIE);
				}
			...
			lsvTabDocRetPropr.Items.Add(varListItem);
		}
		lsvTabDocRetPropr.EndUpdate();
		lsvTabDocRetPropr.ResumeLayout();
	...
	{
		foreach (ListViewItem varListItem in lsvTabDocRetPropr.Items)
		{
	...
	{
		return await clsScreenGeral.funcExportToCsvAsync(lsvTabDocRetPropr, onProgressChange);
	}
	...
	{
		return clsFunction.funcGetListViewItens(lsvTabDocRetPropr, pAllItens: false, pFocused, pChecked);
	}
	...
	{
		return lsvTabDocRetPropr.Items.Count;
	}
	...
	{
		if (lsvTabDocRetPropr != null)
		{
			IList varObjectsOnView = lsvTabDocRetPropr.CheckedItems;
			if (varObjectsOnView.Count <= 0)
			{
				varObjectsOnView = lsvTabDocRetPropr.Items;
			}
	...
	{
		clsHelpService.funcCallTipReportNFeDevolutionPropriaAsync();
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Monitor.frmTabDocRetPropr));
		...
		this.lsvTabDocRetPropr = new System.Windows.Forms.ListView();
		...
		this.pnContent.Controls.Add(this.lsvTabDocRetPropr);
		...
		this.lsvTabDocRetPropr.Alignment = System.Windows.Forms.ListViewAlignment.Left;
		this.lsvTabDocRetPropr.AllowColumnReorder = true;
		this.lsvTabDocRetPropr.Anchor = System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Left | System.Windows.Forms.AnchorStyles.Right;
		this.lsvTabDocRetPropr.CheckBoxes = true;
		this.lsvTabDocRetPropr.Columns.AddRange(new System.Windows.Forms.ColumnHeader[25]
		{
		...
		this.lsvTabDocRetPropr.ContextMenuStrip = this.contextMenuDocs;
		this.lsvTabDocRetPropr.Font = new System.Drawing.Font("Verdana", 8.25f, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, 0);
		this.lsvTabDocRetPropr.FullRowSelect = true;
		this.lsvTabDocRetPropr.HideSelection = false;
		this.lsvTabDocRetPropr.Location = new System.Drawing.Point(0, 0);
		this.lsvTabDocRetPropr.Name = "lsvTabDocRetPropr";
		this.lsvTabDocRetPropr.ShowItemToolTips = true;
		this.lsvTabDocRetPropr.Size = new System.Drawing.Size(784, 116);
		this.lsvTabDocRetPropr.SmallImageList = this.ImageListDocs;
		this.lsvTabDocRetPropr.TabIndex = 8;
		this.lsvTabDocRetPropr.UseCompatibleStateImageBehavior = false;
		this.lsvTabDocRetPropr.View = System.Windows.Forms.View.Details;
		this.lsvTabDocRetPropr.ColumnClick += new System.Windows.Forms.ColumnClickEventHandler(lstView_ColumnClick);
		this.lsvTabDocRetPropr.ColumnReordered += new System.Windows.Forms.ColumnReorderedEventHandler(lstView_ColumnReordered);
		this.lsvTabDocRetPropr.ColumnWidthChanged += new System.Windows.Forms.ColumnWidthChangedEventHandler(lstView_ColumnWidthChanged);
		this.lsvTabDocRetPropr.ItemChecked += new System.Windows.Forms.ItemCheckedEventHandler(lstView_ItemChecked);
		this.lsvTabDocRetPropr.DoubleClick += new System.EventHandler(lstView_DoubleClick);
		...
		this.lknAction.Text = "NFe de devolução própria";
		...
		this.lbText01.Text = "Lista todas as NFe de devolução com CNPJ próprio que seus fornecedores emitiram quando a sua empresa recusou o recebimento de uma mercadoria.";
		...
		this.lbTitle01.Text = "NFes de devolução própria é ativado nos \r\nplanos Avançado e Enterprise.";
		...
		base.Name = "frmTabDocRetPropr";
		this.Text = "NFes de devolução própria";
		base.Load += new System.EventHandler(frmTabDocRetPropr_Load);
