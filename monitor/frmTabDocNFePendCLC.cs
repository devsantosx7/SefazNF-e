...
public class frmTabDocNFePendCLC : Form
{
	...
	private clsSrvTabDocNFePendCLC _SqlTabData = new clsSrvTabDocNFePendCLC();
	...
	private ListView lsvTabDocNFePendCLC;
	...
	public frmTabDocNFePendCLC(string pTitle, Color pColumnColor, string pFeatExtId)
	{
		InitializeComponent();
		Text = pTitle;
	...
	private async void frmTabDocNFePendCLC_Load(object sender, EventArgs e)
	{
		...
		clsFunction.funcSetColumnsConfiguration(this, lsvTabDocNFePendCLC);
		_ColumnsLoaded = true;
		lsvTabDocNFePendCLC.Columns[0].ImageIndex = 16;
		...
		if (_IsLocked)
		{
			...
			lsvTabDocNFePendCLC.Dock = DockStyle.None;
		...
		{
			lsvTabDocNFePendCLC.Dock = DockStyle.Fill;
	...
	private async Task<clsReturn> funcShowDataAsync(clsDataFilter pclsFilter, List<Document> pDocList)
	{
		...
		lsvTabDocNFePendCLC.BeginUpdate();
		lsvTabDocNFePendCLC.SuspendLayout();
		lsvTabDocNFePendCLC.Items.Clear();
		int varCounter = 0;
		...
		{
			lsvTabDocNFePendCLC.ShowGroups = false;
			lsvTabDocNFePendCLC.Groups.Clear();
		}
		else
		{
			lsvTabDocNFePendCLC.ShowGroups = true;
		}
		foreach (Document varclsDocument in pDocList)
		{
			...
			if (pclsFilter.GroupByPartner)
			{
				...
				if (!varGroupByText.Equals(varPartnerIdnt))
				{
					...
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varPartnerIdnt, varGroupHeaderText);
				}
			...
			else if (pclsFilter.GroupByDate)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varclsDocument.DtAut, varclsDocument.DtAut);
				}
			...
			else if (pclsFilter.GroupByYearMonth)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varAnoMes, varAnoMes);
				}
			...
			else if (pclsFilter.GroupByState)
			{
				if (!varGroupByText.Equals(varclsDocument.cUF))
				{
					...
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varclsDocument.cUF, varGroupHeaderText2);
				}
			...
			else if (pclsFilter.GroupByModel)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varModel, varModel);
				}
			...
			else if (pclsFilter.GroupByTpDoc)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varTipoDoc, varTipoDoc);
				}
			...
			else if (pclsFilter.GroupByNatOper)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varNatOper, varNatOper);
				}
			...
			else if (pclsFilter.GroupByCFOP)
			{
				...
				{
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varDocCFOP, varDocCFOP);
				}
			...
			else if (pclsFilter.GroupByFilial)
			{
				...
				if (!varGroupByText.Equals(varFilialId))
				{
					...
					varListGroup = lsvTabDocNFePendCLC.Groups.Add(varFilialId, varGroupHeaderText3);
				}
			...
			lsvTabDocNFePendCLC.Items.Add(varListItem);
		}
		lsvTabDocNFePendCLC.EndUpdate();
		lsvTabDocNFePendCLC.ResumeLayout();
	...
	{
		foreach (ListViewItem varListItem in lsvTabDocNFePendCLC.Items)
		{
	...
	{
		return await clsScreenGeral.funcExportToCsvAsync(lsvTabDocNFePendCLC, onProgressChange);
	}

	private List<ListViewItem> funcGetObjList(bool pFocused, bool pChecked)
	{
		return clsFunction.funcGetListViewItens(lsvTabDocNFePendCLC, pAllItens: false, pFocused, pChecked);
	}
	...
	{
		return lsvTabDocNFePendCLC.Items.Count;
	}
	...
	private async void lstView_DoubleClick(object sender, EventArgs e)
	{
	...
	{
		clsHelpService.funcCallTipReportNFePendCollectAsync();
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Monitor.frmTabDocNFePendCLC));
		...
		this.lsvTabDocNFePendCLC = new System.Windows.Forms.ListView();
		...
		this.pnContent.Controls.Add(this.lsvTabDocNFePendCLC);
		...
		this.lsvTabDocNFePendCLC.Alignment = System.Windows.Forms.ListViewAlignment.Left;
		this.lsvTabDocNFePendCLC.AllowColumnReorder = true;
		this.lsvTabDocNFePendCLC.Anchor = System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Left | System.Windows.Forms.AnchorStyles.Right;
		this.lsvTabDocNFePendCLC.CheckBoxes = true;
		this.lsvTabDocNFePendCLC.Columns.AddRange(new System.Windows.Forms.ColumnHeader[18]
		{
		...
		this.lsvTabDocNFePendCLC.ContextMenuStrip = this.contextMenuDocs;
		this.lsvTabDocNFePendCLC.Font = new System.Drawing.Font("Verdana", 8.25f, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, 0);
		this.lsvTabDocNFePendCLC.FullRowSelect = true;
		this.lsvTabDocNFePendCLC.HideSelection = false;
		this.lsvTabDocNFePendCLC.Location = new System.Drawing.Point(0, 0);
		this.lsvTabDocNFePendCLC.Name = "lsvTabDocNFePendCLC";
		this.lsvTabDocNFePendCLC.ShowItemToolTips = true;
		this.lsvTabDocNFePendCLC.Size = new System.Drawing.Size(784, 116);
		this.lsvTabDocNFePendCLC.SmallImageList = this.ImageListDocs;
		this.lsvTabDocNFePendCLC.TabIndex = 8;
		this.lsvTabDocNFePendCLC.UseCompatibleStateImageBehavior = false;
		this.lsvTabDocNFePendCLC.View = System.Windows.Forms.View.Details;
		this.lsvTabDocNFePendCLC.ColumnClick += new System.Windows.Forms.ColumnClickEventHandler(lstView_ColumnClick);
		this.lsvTabDocNFePendCLC.ColumnReordered += new System.Windows.Forms.ColumnReorderedEventHandler(lstView_ColumnReordered);
		this.lsvTabDocNFePendCLC.ColumnWidthChanged += new System.Windows.Forms.ColumnWidthChangedEventHandler(lstView_ColumnWidthChanged);
		this.lsvTabDocNFePendCLC.ItemChecked += new System.Windows.Forms.ItemCheckedEventHandler(lstView_ItemChecked);
		this.lsvTabDocNFePendCLC.DoubleClick += new System.EventHandler(lstView_DoubleClick);
		...
		this.lknAction.Text = "NFe pendentes de coleta";
		...
		this.lbText01.Text = "Lista as NFe emitidas por terceiros com frete sob a responsabilidade da sua empresa (FOB) e que ainda não tiveram o devido CTe emitido.";
		...
		this.lbTitle01.Text = "NFe pendentes de coleta é ativado nos \r\nplanos Avançado e Enterprise.";
		...
		base.Name = "frmTabDocNFePendCLC";
		this.Text = "NFes pendente de coleta";
		base.Load += new System.EventHandler(frmTabDocNFePendCLC_Load);
